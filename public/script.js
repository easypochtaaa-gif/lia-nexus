document.addEventListener('DOMContentLoaded', () => {
    const terminal = document.getElementById('chat') || document.getElementById('terminal');
    const terminalInput = document.getElementById('chat-input') || document.getElementById('terminal-input');
    ...
    if (window.Telegram && window.Telegram.WebApp) {
        ...
        if (tg.colorScheme === 'light') {
            document.body.classList.add('light-theme'); // Can be expanded with CSS
        }
        addTerminalLine("[TMA] Инициализация среды Telegram Web App завершена.", "sync");
    }
    ...
    if (micBtn) {
        ...
        if (SpeechRecognition) {
            ...
            recognition.onstart = function() {
                micBtn.style.color = '#ff0055';
                addTerminalLine("[MIC] Слушаю...", "sync");
            };

            recognition.onresult = function(event) {
                const speechResult = event.results[0][0].transcript;
                terminalInput.value = speechResult;
                handleCommand(speechResult);
            };

            recognition.onerror = function(event) {
                micBtn.style.color = 'var(--text-dim)';
                addTerminalLine("[MIC] Ошибка или распознавание остановлено.", "error");
            };
